...
def save_measurement(topic, data):
    ...
    """, (data.get("group_id"), data["device_id"], data["sensor"], data["value"], data.get("unit"), data["ts_ms"], data.get("seq"), topic))
    conn.commit()
    cur.close()
    conn.close()
...
def save_status(topic, data):
    ...
    cur.execute("""
        ...
        VALUES (%s, %s, %s, %s, %s, %s)
    """, (data.get("group_id"), data["device_id"], data["status"], data["ts_ms"], data.get("seq"), topic))
    conn.commit()
    cur.close()
    conn.close()
...
def on_message(client, userdata, msg):
    try:
        ...
        if MES_TYPE not in data: return
